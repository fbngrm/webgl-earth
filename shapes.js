...
Sphere = function(gl, r, N, M) {
	...
	for(var i=1; i<=N; i++){
		for(var j=1; j<=M; j++){
			...
			getSphereVertex(r, i, j, N, M, vposition, vnormals, textureCoords);
			getSphereVertex(r, i-1, j, N, M, vposition, vnormals, textureCoords);
			getSphereVertex(r, i-1, j-1, N, M, vposition, vnormals, textureCoords);
			
			getSphereVertex(r, i, j, N, M, vposition, vnormals, textureCoords);
			getSphereVertex(r, i, j-1, N, M, vposition, vnormals, textureCoords);
			getSphereVertex(r, i-1, j-1, N, M, vposition, vnormals, textureCoords);
    ...
    // add normals to the shape
    this.shape.addVertexAttribute(gl, "vertexNormal",    gl.FLOAT, 3, new Float32Array(vnormals));
    
    // add texture coordinates
    this.shape.addVertexAttribute(gl, "vertexTexCoord", gl.FLOAT, 2, new Float32Array(textureCoords));
    
    // add colors to the shape
    this.shape.addVertexAttribute(gl, "vertexColor",    gl.FLOAT, 3, new Float32Array(vcolor));
}  

function getSphereNormals(){
	
}

// calc the position for a vertex in a sphere
function getSphereVertex(r, i, j, N, M, vposition, vnormals, textureCoords){
